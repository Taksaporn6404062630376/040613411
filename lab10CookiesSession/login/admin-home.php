...
$stockQuery = $pdo->prepare("SELECT pid, pname, stock FROM product");
$stockQuery->execute();

echo "<h2>สินค้าคงเหลือ</h2>";

while ($stockRow = $stockQuery->fetch()) {
    $productid = $stockRow['pid'];
    $productName = $stockRow['pname'];
    $stock = $stockRow['stock'];
    echo "ID{$stockRow['pid']}: {$stockRow['pname']} {$stockRow['stock']} ชิ้น<br>";
}
